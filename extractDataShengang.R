...
file.exists("chlA_ShengangShengang.txt")     # caution new data will be appended to this file if it already exists
# file.rename("MODISA_sstShengang.txt","MODISA_sst.old")
file.remove("chlA_ShengangShengang.txt")
...
for (i in 1:lf) {
  ...
  # save csv file
  fe<-file.exists("chlA_ShengangShengang.txt")
  write.table(dat.varSA,"chlA_ShengangShengang.txt",row.names=FALSE,col.names=!fe,sep="\t",dec=".",append=fe)
  # close netCDF
  nc_close(data)
  # clean workspace
...
# variable
var<-"sst"
file.exists("MODISA_sstShengang.txt")     # caution new data will be appended to this file if it already exists
# file.rename("MODISA_sstShengang.txt","MODISA_sst.old")
file.remove("MODISA_sstShengang.txt")
...
for (i in 1:list_sst_files) {
  ...
  # save csv file
  fe<-file.exists("MODISA_sstShengang.txt")
  write.table(dat.varSA,"MODISA_sstShengang.txt",row.names=FALSE,col.names=!fe,sep="\t",dec=".",append=fe)
  # close connection
  nc_close(data)
  # clean workspace
  rm(data,lon,lat,value,unit,dat.var,dat.varSAtmp,dateini,dateend,datemean,year,month,dat.varSA,fe)
...
# variable
var<-"smap_sss"
file.exists("MODISA_salShengang.txt")     # caution new data will be appended to this file if it already exists
# file.rename("MODISA_sstShengang.txt","MODISA_sst.old")
file.remove("MODISA_salShengang.txt")
...
for (i in 1:list_sal_files) {
  ...
  names(dat.varSA)<-c("lon","lat","value","unit","var", "date")
  # save csv file
  fe<-file.exists("MODISA_salShengang.txt")
  write.table(dat.varSA,"MODISA_salShengang.txt",row.names=FALSE,col.names=!fe,sep="\t",dec=".",append=fe)
  # close connection
  nc_close(data)
  # clean workspace
  rm(data,lon,lat,value,unit,dat.var,dat.varSAtmp,dateini,dateend,datemean,year,month,dat.varSA,fe)
...
MODISA_salShengang <- MODISA_salShengang %>% filter(!is.na(value))
salShengangData <- ddply(MODISA_salShengang, .(date), summarize,  mean = mean(value), max = max(value), min = min(value))
glimpse(salShengangData)
salShengangData$date <- as.Date(salShengangData$date)
write.csv(salShengangData,"F:/2019_2020/salShengangData.csv", row.names = FALSE)

chlA_ShengangShengang <- chlA_ShengangShengang %>% filter(!is.na(value))
chlAData <- ddply(chlA_ShengangShengang, .(date), summarize,  mean = mean(value, na.rm=T), 
